...
class LandingController:
    ...
    def __init__(self):
        ...
        self.Y = PID()
        self.Z = PID()
        self.lastTime = self.get_ros_time()
        self.target = None
    ...
    def update(self, state, velocity):
        ...
        if (self.target is None):
            self.node.get_logger().warn("Target position for landing controller is none.")
            return None
        # simplify variables a bit
        time = state.header.stamp.sec + state.header.stamp.nanosec / 1e9
